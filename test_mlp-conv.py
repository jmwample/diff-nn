...
class Net(nn.Module):
    ...
    def split(self):
        return (self.state_dict()['fcc.weight'], self.state_dict()['fcc.bias'])
...
class RNet(nn.Module):
    ...
    def interm_to_input(self, x):
        ...
        y = F.relu(self.conv1( x.view(-1, 1,28,28) ))
        z, ind = self.mp1(y)
        h1 = F.relu(self.conv2(z))
...
class CNet(nn.Module):
    ...
    def forward(self, x):
        y = F.relu( self.fc1(x.view(-1, 784)) )
        return y
        ...
        for i, (data, _) in enumerate(test_loader):
            ...
            if i == 0:
                ...
                save_image(comparison.cpu(),
                         'results/reconstruction_' + str(epoch) + '_lc.png', nrow=n)
...
def main():
    ...
    # Evaluate
    with open("stats/client_times_lc", 'w') as csvf:
